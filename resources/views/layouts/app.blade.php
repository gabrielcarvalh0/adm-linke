<x-layouts.base>
    ...
    @auth()
        ...
        @elseif (in_array(request()->route()->getName(),['profile', 'my-profile'],))
            ...
            @elseif (in_array(request()->route()->getName(),['store', 'store-products'],))
            <div class="main-content position-relative bg-gray-100">
                @include('layouts.navbars.auth.nav-store')
                <div class="vh-100">
                    {{ $slot }}
                    @include('layouts.footers.auth.navfooter')
                </div>
            </div>
    ...
    @guest
        ...
        @elseif (!auth()->check() && in_array(request()->route()->getName(),['sign-up'],))
            ...
            @elseif (in_array(request()->route()->getName(),['store'],))
            <div class="main-content position-relative bg-gray-100">
                @include('layouts.navbars.auth.nav-profile')
                <div>
                    {{ $slot }}
